<template>
	<view>
		<custom-nav-bar title="历史上报" :fixed="false">
		...
		<view class="history-list-item">
			<five-history-item v-for="(item,index) in listData" :key="index" @viewDetail="viewDetail(item)"></five-history-item>
		</view>
	...
	export default {
		data() {
			return {
				...
				page:1,
				pageSize:6,
				listData:[{name:"",id:1},{name:"",id:1},{name:"",id:1},{name:"",id:1},{name:"",id:1}],
				loadingText:"正在加载数据···"
			};
		},
		...
		methods:{
			...
			viewDetail(item){
				this.$navigateTo(`./five-history-detail?id=${item.id}`)
			},
